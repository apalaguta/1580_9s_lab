...
def naz_minut(minuta):
        if minuta >= 5 and minuta <= 20:
            ...
            return(str(minuta) + " минут")
def naz_chas(chasik):
    if chasik >= 0 and chasik < 6:
        vrem_sutok = 0
    elif chasik >= 6 and chasik < 12:
        vrem_sutok = 1     
    elif chasik >= 12 and chasik < 18:
        vrem_sutok = 2
    elif chasik >= 18 and chasik < 24:
        vrem_sutok = 3
    ...
    elif chasik >= 2 and chasik <= 4:
        return(str(chasik)+" часа",vrem_sutok)
    else:
        return(str(chasik)+" часов",vrem_sutok)

vremina_sutok=("ночи","утра","дня","вечера")
polovinki=("первого","второго","третьего","четвертого","пятого","шестого","седьмого","восьмого","девятого","десятого","-одиннацатого","двенадцатого")
...
if chas >= 0 and chas < 24 and minut >= 0 and minut < 60:
    if minut == 0:
        ...
        param_chas = naz_chas((chas + 1) % 24)[1]
        chas = chas % 12
        print("пол" + polovinki[chas],vremina_sutok[param_chas])
